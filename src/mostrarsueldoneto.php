...
<?php if ($resultado && mysqli_num_rows($resultado) > 0): while ($fila = mysqli_fetch_assoc($resultado)): ?><tr><td><strong><?php echo htmlspecialchars($fila['periodo']); ?></strong></td><td><?php echo htmlspecialchars($fila['apellido'] . ', ' . $fila['nombre']); ?></td><td><?php echo htmlspecialchars($fila['DNI']); ?></td><td>$<?php echo number_format((float)$fila['salario'], 2, ',', '.'); ?></td><td class="row-actions"><a class="danger-action" data-confirm-delete="liquidation" href="bajadesdemostrar.php?id_registro=<?php echo urlencode($fila['id_registro']); ?>&amp;DNI=<?php echo urlencode($fila['DNI']); ?>">Eliminar</a></td></tr><?php endwhile; else: ?><tr><td colspan="5" class="empty-state">No encontramos liquidaciones con esos filtros.</td></tr><?php endif; ?>
